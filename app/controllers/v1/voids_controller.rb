class V1::VoidsController < V1::BaseController
  ...
  def search
    if params[:type]
      ...
      order_items = OrderItem.joins(:product, order: :table)
                    .joins("LEFT JOIN users ON users.id = order_items.void_by LEFT JOIN profiles ON profiles.user_id = users.id")
                    .where('void_quantity > 0')
